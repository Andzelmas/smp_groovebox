...
int main(){
	...
	if(cx_curr && selected_cx){
          // highlight the cx_curr context if it is the same as the selected_cx
          // (can happen if app_intrf removes all of cx_curr children)
          ...
          for (unsigned int i = 0; i < count; i++) {
            ...
            if (nav_cx_display_name_return(app_intrf, cx_child, display_name,
                                           MAX_PARAM_NAME_LENGTH) == 1) {
