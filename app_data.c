...
typedef struct _app_info {
  ...
  unsigned int is_processing; // is the main jack function processing, should be
                              // touched only on [audio-thread]
} APP_INFO;
...
void *app_data_child_return(void *parent_data, uint16_t parent_type,
  ...
  if (parent_type == USER_DATA_T_PLUGINS) {
    ...
    if (idx == 0) {
	//TODO would be logical to make a separate button to add lv2 and clap plugins
    ...
    if (idx > 0) {
      ...
      if (plugins_list_item_user_data) {
        *return_type = USER_DATA_T_PLUGINS_LV2_LIST_ITEM;
		plug_plugin_list_item_name(plugins_list_item_user_data, return_name, return_name_len);
...
char **app_plug_get_plugin_names(APP_INFO *app_data, unsigned int *names_size,
                                 unsigned char **return_plug_types) {
  if (!app_data)
    return NULL;
  ...
  char **clap_names =
      clap_plug_return_plugin_names(app_data->clap_plug_data, &clap_size);
  unsigned char *plugin_types =
      malloc(sizeof(unsigned char) * (lv2_size + clap_size));
  ...
  for (int i = 0; i < (lv2_size + clap_size); i++) {
    ...
    if (!cur_name)
      continue;
    char **temp_names =
        realloc(all_names, sizeof(char *) * (total_names_size + 1));
    if (!temp_names) {
      free(cur_name);
      continue;
    }
    ...
    if (names == clap_names)
      plugin_types[total_names_size] = CLAP_plugin_type;
    total_names_size += 1;
  }
...
void *app_plug_presets_iterate(APP_INFO *app_data, unsigned char cx_type,
                               unsigned int idx, uint32_t iter) {
  if (!app_data)
    return NULL;
...
int app_plug_plugin_presets_get_short_name(APP_INFO *app_data,
                                           unsigned char cx_type,
                                           void *preset_info, char *return_name,
                                           uint32_t name_len) {
  if (!app_data)
    return -1;
  if (cx_type == Context_type_Plugins)
    return plug_plugin_preset_short_name(app_data->plug_data, preset_info,
                                         return_name, name_len);
  if (cx_type == Context_type_Clap_Plugins) {
    return clap_plug_presets_name_return(app_data->clap_plug_data, preset_info,
                                         return_name, name_len);
  }

  return -1;
}
int app_plug_plugin_presets_get_full_path(APP_INFO *app_data,
                                          unsigned char cx_type,
                                          void *preset_info, char *return_path,
                                          uint32_t path_len) {
  if (!app_data)
    return -1;
  if (cx_type == Context_type_Plugins)
    return plug_plugin_preset_path(app_data->plug_data, preset_info,
                                   return_path, path_len);
  if (cx_type == Context_type_Clap_Plugins) {
    return clap_plug_presets_path_return(app_data->clap_plug_data, preset_info,
                                         return_path, path_len);
  }

  return -1;
}
int app_plug_plugin_presets_categories_iterate(
    APP_INFO *app_data, unsigned char cx_type, void *preset_info,
    char *cur_category, uint32_t cat_len, uint32_t iter) {
  if (!app_data)
    return -1;
  if (cx_type == Context_type_Plugins)
    ...
    return -1;
  if (cx_type == Context_type_Clap_Plugins) {
    return clap_plug_presets_categories_iterate(
        app_data->clap_plug_data, preset_info, cur_category, cat_len, iter);
  }

  return -1;
}
void app_plug_presets_clean(APP_INFO *app_data, unsigned char cx_type,
                            void *preset_info) {
  if (!app_data)
    return;
  if (cx_type == Context_type_Plugins) {
...
int app_plug_load_preset(APP_INFO *app_data, const char *preset_uri,
                         unsigned int cx_type, const int plug_id) {
  if (!app_data)
    return -1;
  if (!preset_uri)
    return -1;
  ...
  if (cx_type == Context_type_Clap_Plugins)
    return_val = clap_plug_preset_load_from_path(app_data->clap_plug_data,
                                                 plug_id, preset_uri);

  return return_val;
}

int app_plug_init_plugin(APP_INFO *app_data, const char *plugin_uri,
                         unsigned char cx_type, const int id) {
  if (!plugin_uri)
    return -1;
  if (cx_type == Context_type_Plugins)
    return plug_load_and_activate(app_data->plug_data, plugin_uri, id);
  if (cx_type == Context_type_Clap_Plugins)
    return clap_plug_load_and_activate(app_data->clap_plug_data, plugin_uri,
                                       id);
  return -1;
}
...
int app_param_set_value(APP_INFO *app_data, unsigned char cx_type, int cx_id,
                        int param_id, PARAM_T param_value,
                        unsigned char param_op) {
  if (!app_data)
    return -1;
  PRM_CONTAIN *param_cont =
      app_get_context_param_container(app_data, cx_type, cx_id);
  if (!param_cont)
    return -1;
  return param_set_value(param_cont, param_id, param_value, NULL, param_op, 0);
}

PARAM_T app_param_get_increment(APP_INFO *app_data, unsigned char cx_type,
                                int cx_id, int param_id) {
  if (!app_data)
    return -1;
  PRM_CONTAIN *param_cont =
      app_get_context_param_container(app_data, cx_type, cx_id);
...
int app_param_get_ui_name(APP_INFO *app_data, unsigned char cx_type, int cx_id,
                          int param_id, char *name, uint32_t name_len) {
  if (!app_data)
    return -1;
  PRM_CONTAIN *param_cont =
      app_get_context_param_container(app_data, cx_type, cx_id);
...
PARAM_T app_param_get_value(APP_INFO *app_data, unsigned char cx_type,
                            int cx_id, int param_id) {
  if (!app_data)
    return -1;
  PRM_CONTAIN *param_cont =
...
int app_param_id_from_name(APP_INFO *app_data, unsigned char cx_type, int cx_id,
...
unsigned int app_param_get_value_as_string(APP_INFO *app_data,
                                           unsigned char cx_type, int cx_id,
                                           int param_id, char *ret_string,
                                           uint32_t string_len) {
  if (!app_data)
    return 0;
  PRM_CONTAIN *param_cont =
  ...
  if (!param_cont)
    return 0;
  return param_get_value_as_string(param_cont, param_id, ret_string,
                                   string_len);
}

int app_param_return_all_as_string(APP_INFO *app_data, unsigned char cx_type,
                                   int cx_id, char ***param_names,
                                   char ***param_vals,
  ...
  *param_num = param_return_num_params(param_cont, 0);
  ...
  for (int i = 0; i < *param_num; i++) {
    ...
    if (param_get_name(param_cont, i, cur_name, MAX_PARAM_NAME_LENGTH) != 1)
      continue;
...
static void app_make_name_from_file_cx_id(APP_INFO *app_data, char **in_path,
                                          int cx_id, unsigned int add_id) {
  if (!*in_path)
    return;
...
const char **app_context_return_names(APP_INFO *app_data, unsigned char cx_type,
                                      int *cx_num) {
  if (!app_data)
    return NULL;
...
char *app_return_cx_name(APP_INFO *app_data, unsigned char cx_type, int cx_id,
  ...
  // if this is the sampler, return the sample file path and make the name from
  // that
  if (cx_type == Context_type_Sampler) {
  ...
  // if this is the plugin context, return the plugin name from the plugin lilv
  // name
  if (cx_type == Context_type_Plugins) {
...
int app_disconnect_ports(APP_INFO *app_data, const char *source_port,
...
int app_connect_ports(APP_INFO *app_data, const char *source_port,
                      const char *dest_port) {
  if (!app_data)
    return -1;
  if (!app_data->trk_jack)
...
const char **app_return_ports(APP_INFO *app_data, const char *name_pattern,
                              unsigned int type_pattern, unsigned long flags) {
  return app_jack_port_names(app_data->trk_jack, name_pattern, type_pattern,
                             flags);
}
...
// read ring buffers sent from ui to rt thread - this is not for parameter
// values, but for messages like start processing a plugin and similar
...
int trk_audio_process_rt(NFRAMES_T nframes, void *arg) {
  ...
  // stop_processing a plugin, update rt param values etc. if returns a 1 value,
  // this means that is_processing is 0 and the function should not process any
  // contexts a value of -1 means that a fundamental error occured
