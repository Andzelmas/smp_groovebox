...
int app_plug_plugin_presets_get_short_name(APP_INFO *app_data,
    ...
    if (cx_type == Context_type_Clap_Plugins) {
        return clap_plug_presets_name_return(
            app_data->clap_plug_data, preset_info, return_name, name_len);
    }
...
int app_plug_plugin_presets_get_full_path(APP_INFO *app_data,
    ...
    if (cx_type == Context_type_Clap_Plugins) {
        return clap_plug_presets_path_return(
            app_data->clap_plug_data, preset_info, return_path, path_len);
    }
...
int app_param_set_value(APP_INFO *app_data, unsigned char cx_type, int cx_id,
    ...
    if (!param_cont)
        return -1;
    return param_set_value(param_cont, param_id, param_value, NULL, param_op,
                           0);
}
...
char *app_return_cx_name(APP_INFO *app_data, unsigned char cx_type, int cx_id,
    ...
    // if this is the sampler, return the sample file path and make the name
    // from that
    if (cx_type == Context_type_Sampler) {
    ...
    // if this is the plugin context, return the plugin name from the plugin
    // lilv name
    if (cx_type == Context_type_Plugins) {
    ...
    if (cx_type == Context_type_Clap_Plugins) {
        ret_name =
            clap_plug_return_plugin_name(app_data->clap_plug_data, cx_id);
        app_make_name_from_file_cx_id(app_data, &ret_name, cx_id, add_id);
    }
    return ret_name;
}
...
int app_disconnect_ports(APP_INFO *app_data, const char *source_port,
    ...
    if (!app_data->trk_jack)
        return -1;
    return app_jack_disconnect_ports(app_data->trk_jack, source_port,
                                     dest_port);
}
