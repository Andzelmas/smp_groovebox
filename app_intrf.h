...
//exit the cx_curr context, so cx_curr will become the previous cx_curr->cx_parent
int nav_cx_curr_exit(APP_INTRF* app_intrf);
//call the data function to invoke the user_data on the context (user interaction callback in essence)
//during invoke the context structure will not be changed, but it can be marked dirty, so the nav_update() function will know to change the cx structure
//also cx_curr becomes the cx_selected and cx_selected becomes the first cx_selected child.
