...
char *str_find_value_from_name(const char *attrib_names[],
    ...
    char *ret_value = NULL;
    if (attrib_names == NULL || attrib_values == NULL)
        return ret_value;
...
unsigned int str_find_value_to_hex(const char *attrib_names[],
    ...
    from_string = str_find_value_from_name(attrib_names, attrib_values,
                                           find_name, attrib_size);
    if (!from_string)
        goto clean;
...
int str_split_string_delim(const char *in_string, const char *delim,
    ...
    if (before_end >= return_char_sizes)
        before_end = return_char_sizes - 1;
    ...
    if (after_size <= 0)
        ...
        snprintf(after_delim, return_char_sizes, "%s",
                 delim_string + strlen(delim));
...
int str_append_to_string(char **append_string, const char *in_string, ...) {
    ...
    if (!(*append_string)) {
        ...
        *append_string = temp_string;
        ...
        temp_string = realloc(
            *append_string, sizeof(char) * (strlen(*append_string) + ret + 1));
        if (!temp_string)
            return -1;
        *append_string = temp_string;
