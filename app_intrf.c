...
// or  cx_selected. Nav_ functions that exits cx_curr or invokes  cx_selected
// should exit and invoke cx given as arguments and apply the results to the
// group given as an argument. OR these functions should have arguments of
// groups where to exit cx_curr and invoke cx_selected and to what group apply
// the result. This way ui can invoke cx and enter it in one group but update
// the cx_curr on another group and show the children in a different window for
// example.
// TODO SAVING should be on the app_data layer.
// app_intrf calls function with the filename  where to save  and app_data saves
// there the structs as bites. when a file is loaded the app_data creates its
// structure from the file (creates the structs in memory) and marks the root as
// dirty so app_intrf recreates its structure. saving and loading separate
// contexts (plugins, trk and similar) should work the same. user should be able
// to set a file to load on startup.
// TODO When app_intrf checks contexts and finds a cx dirty, will need a special
// function to remove and create the cx children again: go through each child
// with for loop and check their flags if they are eligible for recreation:
// children with INTRF_FLAG_CANT_DIRTY will not be removed. remove the children
// recursively. So even if childrens children will have a flag
// INTRF_FLAG_CANT_DIRTY they will be removed, since their parent is removed and
// there would be a memory leak. in this way if for example structure is
...
// will be marked dirty. Since load_preset is Plugins childrens children it will
// be removed this time and created again.
// TODO when implementing clay or other ui, test mouse clicking; scrolling(would
// be nice to able to scroll any element with contents that do not fit) and
...
typedef struct _cx {
	...
	unsigned int cx_children_last_selected; // the child in cx_children that was
										  // last selected (address put in
...
typedef struct _app_intrf {
    ...
    CX *cx_selected; // the selected or last interacted context, when entering a
                   // context will be the first child
    uint16_t main_user_data_type; // type for the main user_data sturct, the same
                                // type is in cx_root->user_data_type
    void *main_user_data; // main user_data struct for convenience, the same
                        // struct is in cx_root->user_data
    // return the idx child for the parent_user_data. Will return NULL if idx is
    // out of bounds flags returns the flags for this context from the data
    ...
    void *(*data_child_return)(void *parent_user_data, uint16_t parent_type,
                             ...
                             unsigned int idx);
    // data function that updates its internal structures every cycle, should be
    // called first before any navigation
    ...
    // file can be a NULL, a file on the disk, or some other char* that data needs
    // depending on the INTRF_FLAG_
    void (*data_invoke)(void *user_data, uint16_t user_data_type,
                      const char *file);
    // check this user_data for dirty, if it is dirty, need to remove all of its
    // children cx and create them again.
...
static void app_intrf_cx_remove(APP_INTRF *app_intrf, CX *remove_cx) {
  ...
  while (remove_cx->cx_children_count > 0 && safety > 0) {
    ...
    // so next remove the next child in the array, this error will leave
    // cx_children array not popped and with NULLs in some indices, better then
    // memory leak
...
static void app_intrf_cx_children_create(APP_INTRF *app_intrf, CX *parent_cx) {
  ...
  while (child_data) {
    ...
    iter += 1;
    child_data = app_intrf->data_child_return(parent_cx->user_data,
                                              parent_cx->user_data_type,
                                              &child_type, &child_flags, 
                                              child_name, MAX_PARAM_NAME_LENGTH,
                                              iter);
  }
...
APP_INTRF *app_intrf_init() {
  ...
  app_intrf->main_user_data =
      app_init(&(app_intrf->main_user_data_type), &root_flags, root_name, MAX_PARAM_NAME_LENGTH);
  ...
  // create the cx_root
  app_intrf->cx_root =
      app_intrf_cx_create(app_intrf, NULL, app_intrf->main_user_data,
...
static void app_intrf_cx_check_dirty(APP_INTRF *app_intrf, CX *cur_cx) {
  ...
  // since app_intrf_cx_children_create() checks for cx duplicates in the parent_cx
  // cx that are not remove if they have _CANT_DIRTY flag will not be duplicated
...
int nav_cx_curr_exit(APP_INTRF *app_intrf) {
  ...
  if (app_intrf->cx_curr->cx_children_count > 0) {
    ...
    if (app_intrf->cx_curr->cx_children_last_selected <
      ...
      app_intrf->cx_selected =
          app_intrf->cx_curr
              ->cx_children[app_intrf->cx_curr->cx_children_last_selected];
  }
