...
typedef struct _cxcontrol_ui_funcs{
    ...
    int (*subcx_callback)(void* user_data); //called when received a sys message from [audio-thread] to run a subcontext function on the [main-thread]
    int (*subcx_activate_start_process)(void* user_data); //called when received a sys message from [audio-thread] to activate the subcontext and then send a message back to [audio-thread] to start processing it
    int (*subcx_restart)(void* user_data); //called when received a sys message from [audio-thread] to restart the subcontext
}CXCONTROL_UI_FUNCS;
...
//these functions ask the user_data to do something - restart for example. If is_audio_thread == 0, the apropriate user function will be called right away (for example subcx_restart)
